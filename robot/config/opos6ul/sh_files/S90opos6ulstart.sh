...
# Fix entropie pour Dropbear SSH (evite le blocage au key exchange)
# Sur Linux 5.10+, urandom est cryptographiquement sur apres le boot
if [ -c /dev/random ] && [ ! -L /dev/random ]; then
    rm /dev/random
    ln -s /dev/urandom /dev/random
fi

# Relancer Dropbear avec cle ed25519 uniquement (plus rapide sur ARM)
...
ifconfig wlan0 192.168.3.104 netmask 255.255.255.0 up
#carte 3
#ifconfig wlan0 192.168.3.105 netmask 255.255.255.0 up
