...
void AsservEsialR::initAsserv()
{
    ...
    if (motorC_ == NULL)
        motorC_ = new BotMotors(robot_); //CONFIG_MOTORCTRL_BOTMOTORS a activer dans eclipse pour la config de l'asserv
    if (odo_ == NULL)
...
{
    //On arrete le traitement de l'asserv
...
{
    //printf("Reinitialisation de l'asserv...\r\n");
    logger().debug() << "Reinitialisation de l'asserv..." << logs::end;
    stopAsserv();
    //On reprend l'asserv
    initAsserv();
}
...
void AsservEsialR::motion_Line(float dist_mm)
{
    lock();
...
void AsservEsialR::motion_OrbitalTurnRad(float angle_radians, bool forward, bool turnRight)
{
    ...
    motion_RotateRad(effectiveAngle);
