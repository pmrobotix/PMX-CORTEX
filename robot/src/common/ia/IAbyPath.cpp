...
TRAJ_STATE IAbyPath::doPathForwardTo(float xMM, float yMM, bool rotate_ignoring_opponent)
{
	...
	if (found_path != NULL)
	{
		...
		{
			logger().info() << __FUNCTION__ << " PATH NOT FOUND - IMPOSSIBLE " << ", cost=" << found_path->cost
					<< " x,y=" << xMM << " " << yMM << logs::end;
			delete found_path;
...
TRAJ_STATE IAbyPath::doPathBackwardTo(float xMM, float yMM, bool rotate_ignoring_opponent)
{
	...
	if (found_path != NULL)
	{
		...
		for (nodes_it = found_path->path.begin(); nodes_it < found_path->path.end(); nodes_it++)
		{
			...
			if (count != 0)
			{
				logger().info() << __FUNCTION__ << " ======> PATH GOTO BACKWARD " << node->x << "," << node->y
						<< logs::end;
		...
		bool ignore_collision)
{
	...
	while (ts != TRAJ_FINISHED)
	{
		...
		if (byPathfinding)
		{
			...
			ts = doPathForwardTo(xMM, yMM, rotate_ignoring_opponent);
		} else
		{
		...
		if (ts == TRAJ_NEAR_OBSTACLE)
		{
			...
			if (f < nb_near_obstacle)
			{
				robot_->passerv()->resetEmergencyOnTraj("IAbyPath whileMoveForwardTo TRAJ_INTERRUPTED"); //pour autoriser le level de detection 3 puis 4
			}
			//			if (reculOnObstacleMm > 0)
//			{
//				TRAJ_STATE tr = robot_->passerv()->doLineAbs(-reculOnObstacleMm);
		...
		int nb_near_obstacle, int nb_collision, bool byPathfinding, int reculOnObstacleMm, int reculOnCollisionMm)
{
	...
	while (ts != TRAJ_FINISHED)
	{
		if (byPathfinding)
		{
			...
			ts = doPathBackwardTo(xMM, yMM, rotate_ignoring_opponent);
...
TRAJ_STATE IAbyPath::whileMoveRotateTo(float AbsoluteThetaInDegree, int wait_tempo_us, int nb_collision)
{
	...
	//TODO comment ignorer la collision avec l'asserv ?????
