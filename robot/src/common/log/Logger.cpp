...
void logs::Logger::writeMessage(const logs::Level &level, const std::string &message) const
{
    if (isActive(level)) {
...
void logs::end(logs::Logger::LoggerBuffer &buffer)
{
    buffer.end();
}

void logs::flush(logs::Logger::LoggerBuffer &buffer)
{
    buffer.flush();
}
