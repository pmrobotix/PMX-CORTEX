...
inline std::ostream &operator<<(std::ostream& ostr, tstring& tstr)
{
    ostr << tstr.c_str();
    return ostr;
}
...
private:
    typedef std::map<TCHAR, Option, std::less<TCHAR>, std::allocator<std::map<TCHAR, Option>::value_type > > OptionMap;
