...
int CCAx12Teensy::move_1_servo(uint8_t port, uint8_t id, uint16_t eta_time_ms, uint16_t position,
		uint8_t wait_extratime_ms, uint8_t end_keep_torque, uint16_t torque, uint16_t escape_torque)
{
	uint8_t etalow = eta_time_ms & 0xff;
	uint8_t etahigh = (eta_time_ms >> 8);

	uint8_t poslow = position & 0xff;
	uint8_t poshigh = (position >> 8);

	uint8_t torquelow = torque & 0xff;
	uint8_t torquehigh = (torque >> 8);

	uint8_t esclow = escape_torque & 0xff;
	uint8_t eschigh = (escape_torque >> 8);

	uint8_t buffer[14];
	buffer[0] = AXT_MOVE_1_SERVO;
	buffer[1] = port;
	buffer[2] = id;
	buffer[3] = etalow;
	buffer[4] = etahigh;
	buffer[5] = poslow;
	buffer[6] = poshigh;
	buffer[7] = wait_extratime_ms;
	buffer[8] = end_keep_torque;
	buffer[9] = torquelow;
	buffer[10] = torquehigh;
	buffer[11] = esclow;
	buffer[12] = eschigh;
	buffer[13] = computeCRC(buffer, 13);
