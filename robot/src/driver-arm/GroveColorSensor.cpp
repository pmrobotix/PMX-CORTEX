...
unsigned char GroveColorSensor::TSC3414Blue() {
    ...
    if (connected_) {
        ...
        logger().debug() << "blueLow: " << (int) blueLow << " \tblueHigh: " << (int) blueHigh << logs::end;
...
unsigned char GroveColorSensor::TSC3414Green() {
    ...
    if (connected_) {
        ...
        logger().debug() << "greenLow: " << (int) greenLow << " \tgreenHigh: " << (int) greenHigh << logs::end;
...
unsigned char GroveColorSensor::TSC3414Red() {
    ...
    if (connected_) {
        ...
        logger().debug() << "redLow: " << (int) redLow << " \tredHigh: " << (int) redHigh << logs::end;
...
unsigned char GroveColorSensor::TSC3414Clear() {
    ...
    if (connected_) {
        ...
        logger().debug() << "clearH: " << (int) clearLow << "  \tclearL: " << (int) clearHigh << logs::end;
...
void GroveColorSensor::TCS3414Setup(int delay1, int delay2) {
    ...
    if (connected_) {
        ...
        utils::sleep_for_micros(delay1 * 1000);				//14
        ...
        utils::sleep_for_micros(delay2 * 1000); //14 //wait for a moment to allow ADC to initialize*/
    }
}
...
void GroveColorSensor::makePercentage(unsigned char allcolors[], float allmedium[]) { //makes every color a percentage, 100% is the average of the previous 4 values before this is entered.
    for (int i = 0; i < 4; i++) {
...
float GroveColorSensor::CCTCalc(unsigned char allcolors[]) {
    ...
    logger().debug() << "Illuminance: "
            << TCS3414tristimulus_[1]
            << " \tx: "
            << TCS3414chromaticityCoordinates_[0]
            << " \ty: "
            << TCS3414chromaticityCoordinates_[1]
            << " \tCCT:  "
            << CCT
            << " K"
            << logs::end;
...
unsigned char * GroveColorSensor::TCS3414GetColor() {
    ...
    logger().debug() << "Clear: "
            << (int) TCS3414values_[0]
            << " \tRed: "
            << (int) TCS3414values_[1]
            << " \tGreen: "
            << (int) TCS3414values_[2]
            << " \tBlue:  "
            << (int) TCS3414values_[3]
            << logs::end;
...
void GroveColorSensor::CMD(int delayTime) {
    ...
    utils::sleep_for_micros(delayTime * 1000);
}
