...
void SoundDriver::beep(const std::string &args, bool)
{
	logger().info() << "BEEP" << logs::end;
}

void SoundDriver::tone(unsigned frequency, unsigned ms, bool)
{
	logger().info() << "TONE f=" << frequency << " ms=" << ms << logs::end;
}

void SoundDriver::tone(const std::vector<std::vector<float> > &sequence, bool)
{
	logger().info() << "TONE vector size=" << sequence.size() << logs::end;
}

void SoundDriver::play(const std::string &soundfile, bool)
{
	logger().info() << "PLAY " << soundfile << logs::end;
}

void SoundDriver::speak(const std::string &text, bool)
{
